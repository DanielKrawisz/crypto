...
namespace crypto::cipher::stream {
    ...
    struct XChaCha20_stream : cryptopp_stream<CryptoPP::XChaCha20::Encryption> {
        ...
        XChaCha20_stream (
            const cipher::symmetric_key<32> &key,
            const cipher::initialization_vector<24> &iv) {
    ...
    struct HC128_stream : cryptopp_stream<CryptoPP::HC128::Encryption> {
        constexpr static const security Security = security::modern;
        HC128_stream (
            const symmetric_key<16> &key,
            const cipher::initialization_vector<16> &iv) {
    ...
    struct HC256_stream : cryptopp_stream<CryptoPP::HC256::Encryption> {
        ...
        HC256_stream (
            const cipher::symmetric_key<32> &key,
            const cipher::initialization_vector<32> &iv) {
    ...
    struct XChaCha20 {
        ...
        XChaCha20_stream stream (const cipher::symmetric_key<32> &key);
    };

    struct XSalsa20 {
        constexpr static const security Security = security::modern;
        initialization_vector<24> IV;
        XSalsa20_stream stream (const cipher::symmetric_key<32> &key);
    };

    struct Salsa20 {
        constexpr static const security Security = security::modern;
        initialization_vector<8> IV;
        Salsa20_stream stream (const cipher::symmetric_key<32> &key);
    };

    struct HC128 {
        constexpr static const security Security = security::modern;
        initialization_vector<16> IV;
        HC128_stream stream (const cipher::symmetric_key<32> &key);
    };

    struct HC256 {
        constexpr static const security Security = security::modern;
        initialization_vector<32> IV;
        HC256_stream stream (const cipher::symmetric_key<32> &key);
    };

    struct Panama {
        constexpr static const security Security = security::modern;
        initialization_vector<32> IV;
        Panama_stream stream (const cipher::symmetric_key<32> &key);
    };
}
...
namespace crypto {
    ...
    requires (
        ...
        const cipher::initialization_vector<iv_size> &iv) {
        { stream {key, iv} };
    } bytes encrypt (
        const cipher::symmetric_key<key_size> &key,
        const cipher::initialization_vector<iv_size> &iv,
